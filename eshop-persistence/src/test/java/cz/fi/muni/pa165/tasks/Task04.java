...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductsWithCategories(){
		List<Product> found = em.createQuery("SELECT p from Product p JOIN FETCH p.categories where  p.categories IS NOT EMPTY", Product.class).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery("SELECT NEW cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) FROM Product p LEFT JOIN p.categories AS c ON c.name = 'Kitchen' ORDER BY p.name",ProductAndCategory.class).getResultList();
